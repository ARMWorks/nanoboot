		       ...
		       struct ubi_ec_hdr *ec_hdr, int verbose)
{
	...
	if (magic != UBI_EC_HDR_MAGIC) {
		 ...
		 */
		if (verbose) {
			ubi_warn(ubi, "bad magic number at PEB %d: %08lx instead of %08x",
				 pnum, magic, UBI_EC_HDR_MAGIC);
			ubi_dump_ec_hdr(ec_hdr);
		}
		dbg_bld("bad magic number at PEB %d: %08lx instead of %08x",
			pnum, magic, UBI_EC_HDR_MAGIC);
	...
	if (hdr_crc != crc) {
		if (verbose) {
			ubi_warn(ubi, "bad EC header CRC at PEB %d, calculated %#08lx, read %#08lx",
				 pnum, crc, hdr_crc);
			ubi_dump_ec_hdr(ec_hdr);
		}
		dbg_bld("bad EC header CRC at PEB %d, calculated %#08lx, read %#08lx",
			pnum, crc, hdr_crc);
			...
			struct ubi_vid_hdr *vid_hdr, int verbose)
{
	...
	if (hdr_crc != crc) {
		if (verbose) {
			ubi_warn(ubi, "bad CRC at PEB %d, calculated %#08lx, read %#08lx",
				 pnum, crc, hdr_crc);
			ubi_dump_vid_hdr(vid_hdr);
		}
		dbg_bld("bad CRC at PEB %d, calculated %#08lx, read %#08lx",
			pnum, crc, hdr_crc);
		if (!read_err)
			return UBI_IO_BAD_HDR;
		else
			     ...
			     const struct ubi_ec_hdr *ec_hdr)
{
	...
	magic = be32_to_cpu(ec_hdr->magic);
	if (magic != UBI_EC_HDR_MAGIC) {
		ubi_err(ubi, "bad magic %#08lx, must be %#08x",
...
static int self_check_peb_ec_hdr(const struct ubi_device *ubi, int pnum)
{
	...
	hdr_crc = be32_to_cpu(ec_hdr->hdr_crc);
	if (hdr_crc != crc) {
		ubi_err(ubi, "bad CRC, calculated %#08lx, read %#08lx",
			crc, hdr_crc);
		ubi_err(ubi, "self-check failed for PEB %d", pnum);
		ubi_dump_ec_hdr(ec_hdr);
		dump_stack();
			      ...
			      const struct ubi_vid_hdr *vid_hdr)
{
	...
	magic = be32_to_cpu(vid_hdr->magic);
	if (magic != UBI_VID_HDR_MAGIC) {
		ubi_err(ubi, "bad VID header magic %#08lx at PEB %d, must be %#08x",
...
static int self_check_peb_vid_hdr(const struct ubi_device *ubi, int pnum)
{
	...
	hdr_crc = be32_to_cpu(vid_hdr->hdr_crc);
	if (hdr_crc != crc) {
		ubi_err(ubi, "bad VID header CRC at PEB %d, calculated %#08lx, read %#08lx",
			pnum, crc, hdr_crc);
		ubi_err(ubi, "self-check failed for PEB %d", pnum);
		ubi_dump_vid_hdr(vid_hdr);
		dump_stack();
